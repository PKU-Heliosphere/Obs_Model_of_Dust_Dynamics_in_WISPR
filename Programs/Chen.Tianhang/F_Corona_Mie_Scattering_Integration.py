...
def obtain_Mie_scattered_white_light_intersity(lon_angle, lat_angle):
    ...
    l = np.cos(np.deg2rad(lat_angle))*np.cos(np.deg2rad(lon_angle)) - (1/np.tan(np.deg2rad(scatter_angle)))*np.sqrt(1-np.cos(np.deg2rad(lat_angle))**2*np.sin(np.deg2rad(lon_angle))**2) # AU
    cos2betas = 1-l**2*np.sin(np.deg2rad(lat_angle))**2/(1-2*l*np.cos(np.deg2rad(lat_angle))*np.cos(np.deg2rad(lon_angle))+l**2)
...
I = np.zeros((len(lon),len(lat)))
for i in range(0, len(lon)):  # unit: degree.
    for j in range(0, len(lat)):
        I_tmp = obtain_Mie_scattered_white_light_intersity(lon[i],lat[j])
        I[i,j] = I_tmp

[lonv,latv] = np.meshgrid(lon,lat)
I = np.array(I)
plt.pcolormesh(lon,lat, np.log10(np.transpose(I)))
plt.axis('scaled')
...
I = np.zeros((len(lon),1))
for i in range(0, len(lon)):
    I_tmp = obtain_Mie_scattered_white_light_intersity(lon[i],0)
    I[i] = I_tmp
print(I[19])
plt.plot(lon, I)
plt.yscale('log')
