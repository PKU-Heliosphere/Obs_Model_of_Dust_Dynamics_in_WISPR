...
import sunpy.visualization.colormaps as suncm
###########################################
...
streak_number = 10  # The numbers of selected streaks in the WISPR_Inner at observing time.
fits_file_path = ('D://Microsoft Download/Formal Files/data file/FITS/WISPR/WISPR-I_ENC07_L3_FITS/20210112/'
                  'psp_L3_wispr_20210112T213015_V1_1221.fits')
# fits_file_path = ("D:/Microsoft Download/Formal Files/data file/FITS/WISPR/L2_Background/"
#                   "WISPR_ENC07_L2b_FITS/20210117/psp_L2b_wispr_20210117T003027_V1_1211.fits")
psp_3d_model_path = 'D://Microsoft Download/Formal Files/data file/3d_model/ParkerSolarProbe/stl/ParkerSolarProbe.stl'
# output_file_path = 'D://Desktop/Orbit07_WISPR_Impact_Region_statistics/all_impact_origin.txt'
WISPR_pos = np.array([0.865, -0.249, -0.300], dtype=float)  # the position of WISPR onboard PSP in spacecraft frame.
z_TPS = 1.479554  # unit: m. The z coordinate of the upper surface of TPS in S/C frame.
line_flag = False
# line_flag being True means the streaks in WISPR_INNER are parallel to each other,
...
def retrieve_streaks(num_streaks):
    ...
    # plt.imshow(true_data, cmap=my_colormap)
    ...
    for i in range(num_streaks):
        ...
        ax.plot([point_set[i, 0, 0] - 1, point_set[i, 1, 0] - 1],
                [point_set[i, 0, 1] - 1, point_set[i, 1, 1] - 1],
                'r')
        ax.plot([point_set[i, 0, 0] - 1 - 5 * (point_set[i, 1, 0] - point_set[i, 0, 0]), point_set[i, 0, 0] - 1],
                [point_set[i, 0, 1] - 1 - 5 * (point_set[i, 1, 1] - point_set[i, 0, 1]), point_set[i, 0, 1] - 1],
                '--r')
...
def plot_3d_trajectory(fun_center_point, line_orientation, timestr, epoch_time, the_flag, error):
    """
    ...
    """
    trajectory_traces = []
    cos_sc_wispr, _ = spice.spkcpt(np.array([0, 0, 1], dtype=float), 'SPP', 'SPP_WISPR_INNER', et,
                                   'SPP_SPACECRAFT', 'OBSERVER', 'NONE', 'SPP')
    cos_sc_wispr = cos_sc_wispr[2]
    ...
    if ~the_flag:
        trajectory_traces.append(
            go.Scatter3d(
                ...
                mode='lines', opacity=0.8, line=dict(color='orange', width=7), legendgroup='centroid',
                legendgrouptitle=dict(text='Possible position of the cone origin')
            )
        )
        beta = (1.353914 - WISPR_pos[2]) / (the_center_point_3d[2]*2 - WISPR_pos[2]*2)
...
def figure_in_3d(x_beg, x_end, y_beg, y_end, distance):
    """
    This 3d frame is Spacecraft Frame. But all the coordinates input are in WISPR-Inner Frame.
    :param x_beg: the left end of the figure
    :param x_end: the right end of the figure.
    :param y_beg: the top of the figure
    :param y_end: the bottom of the figure
    :param distance: the distance from the figure to WISPR.
            All coordinates with the unit of meter.
    :return: a Plotly trace.
    """
    the_shape = true_data.shape
    nx = the_shape[1]
    ny = the_shape[0]
    x_range = np.linspace(x_beg, x_end, nx)
    y_range = np.linspace(y_beg, y_end, ny)
    x, y = np.meshgrid(x_range, y_range)
    z = np.ones_like(x) * distance
    for fun_i in range(ny):
        for fun_j in range(nx):
            temp_coord, _ = spice.spkcpt([x[fun_i, fun_j], y[fun_i, fun_j], z[fun_i, fun_j]], 'SPP', 'SPP_WISPR_INNER',
                                         a_etime, 'SPP_SPACECRAFT', 'OBSERVER', 'NONE', 'SPP')
            x[fun_i, fun_j], y[fun_i, fun_j], z[fun_i, fun_j] = temp_coord[0:3] + WISPR_pos[0:3]
    wispr_colorscale = matplotlib_to_plotly(my_colormap, 255)
    fun_trace = go.Surface(x=x, y=y, z=z, surfacecolor=true_data, colorscale=wispr_colorscale, showscale=False,
                           cmin=1 * 1e-13, cmax=5 * 1e-12)
    return fun_trace


def matplotlib_to_plotly(cmap, pl_entries, A=0.0001):
    """
    ...
    :param A: the normalized colormap with the form of f(x) = A * (B^x) + C, where f(0) = 0, f(1) = 1. (A, B>0)
    :return:
    """
    h = 1.0/(pl_entries-1)
    pl_colorscale = []
    ...
    for k in range(pl_entries):
        ...
        pl_colorscale.append([A*B**(k*h)+C, 'rgb'+str((the_color[0], the_color[1], the_color[2]))])
...
if __name__ == '__main__':
    ...
    z_dis = 1.5
    fov_x_left = - 960 * 10e-6 * z_dis / 28e-3
    fov_x_right = - fov_x_left
    fov_y_top = - 1024 * 10e-6 * z_dis / 28e-3
    fov_y_bottom = - fov_y_top
    ...
    while i < streak_number:
        ...
        while j < streak_number:
            ...
            j = j + 1
        i = i + 1
    interceptions = np.array(interceptions, dtype='float64')
    the_center_point, error_r = get_vanishing_point(interceptions)
    ...
    # 2.25 is the true size of TPS diameter(smallest), while 0.219372*2 is the size of it in stl file.
    #  ——Tianhang Chen, 2022.3.18
    ...
    plotly_trace_4 = plot_3d_trajectory(the_center_point, orientation_vec_sc, header['DATE-BEG'], et, line_flag, error_r)
    plotly_trace_5 = figure_in_3d(fov_x_left, fov_x_right, fov_y_top, fov_y_bottom, z_dis)
    ...
    plotly_fig.add_trace(plotly_trace_5)
