"""
This sript is to visualize the occurence rate(Not always the impact rate) calculated by WISPR-Inner and FIELDS TDSmax data.
And some other conceptions are introduced(e.g., Kepler distance(Szalay et.al., ApJS-2020)).
"""
...
def read_data(file_path, time_format='%Y-%m-%dT%H:%M:%S'):
    ...
    all_num = np.zeros([5, case_num], dtype=float)
    ...
    for fun_i in range(case_num):
        ...
        all_num[3, fun_i] = int(temp_num[3]) / total_num[fun_i] * total_num[fun_i]
...
#     plt.savefig('OUTPUT/output_figure.eps', dpi=300)
    plt.show()
